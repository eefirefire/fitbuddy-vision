...
class RehabSitToStandSession:
    ...
    def route_frame(self, pose_landmarks, frame_w: int, frame_h: int, timestamp: float,
                     settling_window_s: float, source: str = None) -> dict:
        ...
        `source` (e.g. "live", "upload") guards a real, confirmed gap: the
        live stream uses wall-clock timestamps and /upload uses video-
        relative ones. If both ever fed the same shared global session
        (an unusual but possible usage pattern -- nothing previously
        stopped a user starting a live stream and POSTing an upload to the
        same RECORD session), whichever source's frame arrived first would
        anchor _route_started_at in its own time basis, and the other
        source's settling decision would be computed against a
        wildly-mismatched scale, silently corrupting calibration. Once a
        source claims a session (first non-None `source` passed to
        route_frame), frames tagged with a DIFFERENT source are ignored
        outright rather than allowed to corrupt the shared timestamp
        basis -- this session belongs to whichever source got there first
        until the next reset/advance.

        Returns the process_frame-shaped result dict ({} while settling,
        inactive, given no pose_landmarks, or given a mismatched source)."""
        if not self._active() or pose_landmarks is None:
            return {}
        if source is not None:
            if self._frame_source is None:
                self._frame_source = source
            elif source != self._frame_source:
                return {}
...
# under them, mid-request). upload_video holds this lock for its ENTIRE
# request, not just per-frame -- an earlier version only locked each
# individual route_frame call, leaving a real gap where a concurrent
# /advance could land BETWEEN two frames of a long upload and reset state
# mid-video. Holding it for the whole upload means /advance, /reset, and
# the live stream's own frames block until the upload finishes -- correct
# for this module's single-shared-session design, where nothing should be
# mutating session state while a user-initiated upload is still being
# processed anyway. route_frame's `source` parameter is the other half of
# this: it stops two DIFFERENT frame sources (e.g. live + upload) from
# ever corrupting each other's settling timestamp basis if they were
# somehow both fed into the same session despite the lock.
...
def upload_video():
    ...
    # Locked for the ENTIRE upload -- not just per-frame. An earlier version
    # only locked each individual route_frame call, leaving a real gap: a
    # concurrent /advance landing BETWEEN two frames of a long upload (these
    # can run 50-280+ seconds against real footage) could reset _candidates
    # mid-video, silently corrupting the rest of that upload's tracking and
    # making the post-loop finalize_recording() below a no-op. Holding the
    # lock for the whole request means /advance, /reset, and the live
    # stream's own route_frame calls block until this upload finishes --
    # acceptable and arguably correct for this single-shared-session demo
    # app, where nothing should be mutating session state while an upload
    # the user is actively waiting on is still being processed anyway.
    with _session_lock:
        try:
